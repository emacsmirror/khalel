...
** Error message =khal exited with non-zero exit code; see buffer ‘*khal-output*’ for details.=
As stated in the error message, open the =*khal-output*= to see the exact cause
of the error.

A likely reason for this error is =khalel= relying on VCALENDAR fields not
supported in the installed =khal= version. Double-check that your version
...
Should a later version than the above mentioned cause this error, then please
report this problem! Include the exact version of =khal= and the contents of the
=*khal-output*= buffer.
* Reporting issues
